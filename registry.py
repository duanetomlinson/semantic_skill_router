...
class SkillRegistry:
    ...
    def search(self, query_text: str, top_k: int = 3) -> list[dict]:
        ...
        for doc in results.docs:
            ...
            # Fetch full JSON doc using the key returned by search
            key = doc.id if isinstance(doc.id, str) else doc.id.decode()
            full_doc = self.r.json().get(key)
            ...
            matches.append({
                ...
                "handler": full_doc.get("handler", ""),
                "default_args": full_doc.get("default_args", {}),
    ...
    def route(self, query_text: str, threshold: float = 0.48) -> dict | None:
        """
